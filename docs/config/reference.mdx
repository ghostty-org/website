...
MacOS users may find this setting doesn't work. If that is the case,
double check that you do not have another config file located at:
`~/Library/Application\ Support/com.mitchellh.ghostty/config`. Deleting any
settings from that file will allow the settings in your `.config/ghostty/config`
file to load.
