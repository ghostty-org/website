...
<Note>
  ...
  impossible to remember. We plan on introducing a `ghostty +new-window` CLI
  command that automates this based on the runtime environment, such as the OS,
  if systemd is available, etc.
</Note>
