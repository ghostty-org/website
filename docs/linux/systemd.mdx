...
<Important>
  Using `gtk-single-instance` or `class` in your configuration file can
  interfere with the proper operation of the `systemd` unit files or D-Bus
  activation. We recommend that you remove them from your configuration file.
</Important>
