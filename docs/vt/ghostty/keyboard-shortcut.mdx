...
`C-j`, `c-j`, and `C-J` are all equivalent. For case-insensitivity comparisons,
...
To go further on case-insensitivity, `C-J` and `C-S-j` would conflict
on a US standard keyboard layout. The exact behavior in this scenario
for which binding is triggered is unspecified, and behavior differs across
GUI frameworks as well. See the [Qt documentation](https://doc.qt.io/qt-6/qkeysequence.html#keyboard-layout-issues)
on keyboard layout issues for more details.
