...
While not a general replacement, this protocol can often be used in place
of feature-rich but more complex input protocols such as
[Kitty Keyboard Protocol](https://sw.kovidgoyal.net/kitty/keyboard-protocol/),
if the terminal application only needs to support keyboard shortcuts
and raw text input[^6].
...
The primary con is that keyboard shortcuts are completely invisible
to the terminal emulator. This means that terminal emulators cannot
provide any UI for keyboard shortcuts, can't detect conflicts with
application or system shortcuts, can't remap keyboard shortcuts
(possibly extremely important for accessibility), etc.

An additional con is added implementation complexity for terminal
programs. Terminal programs have to parse the raw key events and map
them to keyboard shortcuts. They're forced to implement their own
"help" UIs. They have to implement their own configuration mechanisms.

There are also minor cons such as the fact that unnecessary key events
have to be encoded. This is a minor cost but is wasteful CPU cycles
and IO bandwidth. These costs are usually negligible, but they're
...
The most important themes throughout the more detailed list below
are shifting implementation complexity from terminal programs to terminal
emulators, providing a more consistent user experience (that may match
other native applications), and improving accessibility.
...
supports. This is a very useful feature for discoverability. These
UIs are also usually visible to accessibility tools, making terminal
...
users to remap keyboard shortcuts in the system preferences. Note these
are remappings _for specific actions in a specific application_, not
globally. In addition to this utility, the remapping UI is generally
accessible to accessibility tools. And the operating system can often
provide additional features such as automatic syncing across devices.
...
and menu bar for discoverability and ease of use.

## Protocol

A new OSC sequence is introduced to allow terminal programs to advertise
the set of keyboard shortcuts they support. The OSC[^1] sequence is formatted[^2]:

```
OSC 👻 ; KS ; <op> ; <params> ST
```

The `👻` is UTF-8 encoded as `0xF0 0x9F 0x91 0xBB`.
...
**This protocol is not a replacement for existing input protocols.**
This protocol is a high-level API for actions that can be bound
to keyboard shortcuts. It is not a replacement for low-level key or
text input protocols such as
[Kitty Keyboard Protocol](https://sw.kovidgoyal.net/kitty/keyboard-protocol/).
While many applications may be able to use this protocol exclusively,
it is not a replacement for all the functionality that low-level input
protocols provide, much in the same way that GUI frameworks provide
both high-level APIs for keyboard shortcuts and low-level APIs for
keyboard input.
...
Ghostty extensions use `XTGETTCAP` because it is a well-supported[^5]
feature in modern terminal emulators and importantly provides _negative_
...
It can also set a number of scoped options such as program name so
that they don't have to be repeated for every operation.
...
The program name as used in the
[`+` (register)](#+:-register)
operation.

This program name will be used by default for all subsequent
register operations unless overridden by the `p` option explicitly
in the register option or another save operation. This is useful
to avoid repeating the program name for every operation. For terminal
programs with many actions, this can be a significant amount of
bytes saved.

**Option: `r=<1|0>`**

If `1`, output the response ID directly with no OSC wrapping.
Any other values are ignored.

This is useful for scripting environments where it may be
difficult or impossible to capture and parse an OSC sequence,
such as from a shell script.

When raw mode is enabled, the response will be sent as `<id>\n`.
The single trailing newline is so that the script can easily
detect the end of the response (in case a read results in a
partial read). The newline is not part of the ID and should be stripped.

If an error occurs with raw mode enabled, the error will be suppressed
and a blank ID will be sent instead (`\n`).
...
Restoring a saved state will also invalidate any saved states that were
created after the save operation being restored. This is similar to
stack behavior. This allows outer "scopes" to clean up inner scopes
that may have not cleaned up properly (due to being poorly behaved
...
The `l` (label) and `p` (program) options are highly recommended
to aid UI presenations.
...
The group is a human-friendly name categorizing the action.
This is used for display purposes only and it is up to the terminal
emulator to decide how it is used. It is up to the terminal program
to localize it appropriately.

The group must be valid UTF-8 and it must be base64 encoded.

Ghostty uses the group as a prefix for the action in the command
palette. For example, all text related actions might be in a
`text` group and would show up as `text: <label>` in the command
palette. Alternate UIs may separate shortcuts by group under
tabs or other UI elements.
...
The program name is a human-friendly name for the program that is
associated with the action. This is used for display purposes only and
it is up to the terminal emulator to decide how it is used. It
is up to the terminal program to localize it appropriately.

The program name must be valid UTF-8 and it must be base64 encoded.

Having a dedicated field for program name avoids the need to prefix
every action label with the program name manually. And the structured
data can be used to provide a better UI experience.

Ghostty uses the program name visually to distinguish between
built-in actions from program-specific actions. For example, the command
palette shows the program name in a different style next to the action
label.

<Tip>
If a program has many actions, consider specifying the `p` option
as part of the [`>` (save)](#greater:-save) operation. This will
avoid repeating the program name for every operation and can
significantly reduce the number of bytes sent.
</Tip>
...
[W3C KeyboardEvent codes](https://www.w3.org/TR/uievents-code/)[^3]
with the exception of "Unidentified"[^4].

The keys being bound can be any printable character or W3C key codes.
The key doesn't have to be a key that is normally available via the
encodings supported by the terminal. For example, it is possible to bind
`Numpad9` without using Kitty Keyboard Protocol (a key that is not
otherwise distinguishable via any other encoding). The keyboard shortcut
protocol doesn't rely on any specific key encoding protocol.
...
- `M`: Meta (Command on macOS, Windows key on Windows)
...
Modifiers and keys are both case-insensitive. This means that
`C-j`, `c-j`, and `C-J` are all equivalent, but see the
[section on matching bindings](#matching-bindings) for more information.
For case-insensitivity comparisons, implementations should follow
[Unicode case folding](https://www.w3.org/TR/charmod-norm/#definitionCaseFolding)
rules. Comparison should only use case folding and not full Unicode
normalization.
...
As noted above, any key that is a single-codepoint printable character
will be matched to the character produced by pressing the key. This means that
keyboard layouts are automatically taken into account. For example, for
a binding `C-o`, the key `o` will map to the physical `s` key on a US
standard keyboard with a Dvorak layout.

If you want to bind a key to a specific physical key, you must use
the [W3C KeyboardEvent codes](https://www.w3.org/TR/uievents-code/).
To continue the example above, to map to the physical `o` key the
binding would be `C-KeyO`.

Also note that since printable characters take keyboard layout into
account, some key sequences may be impossible to type on some keyboard layouts.
See the [Qt documentation](https://doc.qt.io/qt-6/qkeysequence.html#keyboard-layout-issues)
for more details.

#### Matching Bindings

A binding is matched to a key event by:

1. Comparing the modifiers pressed to the modifiers in the binding.
2. For physical keys, comparing the key pressed to the key in the binding.
3. For printable characters, comparing the _character ignoring modifiers_
   to the character in the binding using Unicode case folding rules.

An important consequence of the above is that while binding formats
and character comparison is case-insensitive, the modifiers required
to produce the key event may make certain bindings impossible depending
in on the keyboard layout.

For example, `C-_` is impossible to type on a US layout keyboard because
the `_` key requires shift, and pressing shift will cause check 1 above to
fail in the matching algorithm. The equivalent would be `C-S--` on
a US layout keyboard.

Likewise, `C-J` will match `C-j` (lowercase `j`) on a US layout keyboard,
but will not match `C-S-j` (required to produce the uppercase `J` character
on a US layout keyboard). This is because the `C-S-j` key event will
also fail check 1 above. Therefore, despite printable characters being
case-insensitive, modifiers need to be taken into account.

The Qt framework has written
[excellent documentation on keyboard layout issues](https://doc.qt.io/qt-6/qkeysequence.html#keyboard-layout-issues)
as they relate to keyboard shortcuts.
...
`💣` is UTF-8 encoded, the byte sequence is `0xF0 0x9F 0x92 0xA3`.
...
- The operation is not recognized. e.g. `OSC 👻 ; KS ; HUH ST`
...
- Bindings: 4096
- Program name length (`p` option): 256 bytes
...
- 2025-05-12: Clarify more behavior around case-insensitivity.
- 2025-04-29: Reword, reorder the shortcomings and benefits background
  section to have less subjectivity.
- 2025-04-28: Clarify case folding behavior, physical keys, added
  the `p` and `g` options
...
[^1]: OSC is used because it supports strings and is well-supported
by terminal emulators. DCS/APC sequences are less well supported and
dump data to the terminal in Terminal.app.

[^2]: The OSC identifier `👻` is a prefix used for all proprietary
Ghostty sequences.

[^3]: The W3C KeyboardEvent codes are a well-adopted cross-platform
...
[^4]: The "Unidentified" key is a special key that is used when the
key event is not recognized. This is not a valid key for bindings.

[^5]: xterm, foot, Ghostty, iTerm2, Kitty, WezTerm, Contour are
a list of terminals that support it at the time of writing.

[^6]: Terminal applications can use this protocol for keyboard shortcuts
and rely on the bare legacy protocol for chracter input (where characters
are sent as-is on the pty).
