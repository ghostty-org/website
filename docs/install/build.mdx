...
To select a stable build, list the available releases using: 
```sh
git tag --list
```
Now, find the release you want to build from. The latest stable tag is
recommended while the "tip" tag is a rolling development build.

Then checkout the selected tag:
```sh
