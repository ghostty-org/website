...
Ghostty 1.1.0 introduced support for server-side decorations (SSD) on
Linux for Wayland compositors. This allowed Ghostty to look and feel
...
when using a discrete GPUs. Practically, this only affected Intel Macs because
all Apple Silicon Macs have integrated GPUs and Ghostty will always use
...
- Fix a crash that could occur when resetting the terminals with
...
- Fix a crash that could occur with very large OSC8 hyperlinks. There
  are likely other scenarios where this trash could be triggered but
...
At the time of this release, we do not plan to have a 1.1.2 release.
