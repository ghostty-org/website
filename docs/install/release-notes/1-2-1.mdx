...
**13 contributors** over **62 commits.**
...
making Ghostty-provided helper binaries immediately available in new shells
without manual configuration. This simplifies setup and reduces friction when
using Ghostty's tooling.
...
- macOS: allocation error when editing config file no longer crashes. GH-8886
...
- config: add support for `~` expansion in `macos-custom-icon`. GH-9024
- quick-terminal: now stores the last closed size per screen. GH-8796
